...
def display_dashboard(config, stats, wallet_asset, htr, logs, coin_type, status_message=""):
    ...
    dashboard_lines.append(f"{Colors.HEADER}{' ' * ((WIDTH - 12) // 2)}TOOL VIP V8{' ' * ((WIDTH - 12) // 2)}{Colors.RESET}")
    ...
    dashboard_lines.append(f"{Colors.TEXT_LABEL}● Phiên Bản:      {Colors.TEXT_VALUE}LOGIC V8{Colors.RESET}")
    ...
    dashboard_lines.append(f"{BORDER_COLOR}═" * WIDTH)
    if status_message:
        dashboard_lines.append(f"{status_message}")
...
{Colors.RESET}
    """
    print(banner)
    print(f"{Colors.HEADER}Tool VIP V8 - Khởi tạo thành công!{Colors.RESET}\n")
    time.sleep(3)
